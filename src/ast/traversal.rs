...
impl <'a> Node<'a> {
  ...
  /// 
  /// All [Container](`crate::ast::model::Container`) nodes are visited twice, once before visiting their children
  ...
  /// [`NextAction::End`] from the visitor, but the traversal will complete exit visits to any Container nodes
  /// that haven't already received one. [Leaf](`crate::ast::model::Leaf`) nodes are only visited once.
  /// 
  /// ### Parameters
  /// 
  ...
  pub fn traverse(&self, visitor: &impl Visitor) -> NextAction {
      let container = match self.inner() {
          ...
          NodeType::Leaf | NodeType::EOI => return visitor.visit(self, Direction::Entering), 
      };
